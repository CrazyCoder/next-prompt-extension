...
export interface SuggestionState {
	...
	renderMode: RenderMode;
	rearmDelayMs: number;
	rearmTimer: NodeJS.Timeout | undefined;
	editorChangeTimer: NodeJS.Timeout | undefined;
	rearmCheckTimer: NodeJS.Timeout | undefined;
	/**
...
function clearEditorChangeTimer(state: SuggestionState): void {
	if (state.editorChangeTimer !== undefined) {
		clearTimeout(state.editorChangeTimer);
		state.editorChangeTimer = undefined;
	}
}
...
/**
 * Check whether raw input changed an initially-empty editor after the focused
 * component processed it. Non-editing terminal input leaves the suggestion and
 * any pending re-arm intact.
 */
function scheduleEditorChangeCheck(state: SuggestionState): void {
	clearEditorChangeTimer(state);
	state.editorChangeTimer = setTimeout(() => {
		state.editorChangeTimer = undefined;
		if (state.getEditorText().length === 0) return;
...
/**
 * After input on a non-empty editor, re-arm the cached suggestion once deletion
 * settles empty. Only a genuine non-empty-to-empty transition arms the timers
 * (F-09); dismissal via Escape, arrows, or focus does not. The first check waits
 * until the editor processes the key, then polls for a bounded window so slow
 * or chunked deletion can settle before the decision.
 */
function scheduleRearmCheck(
	...
	// Input received after the editor is already empty is not a deletion
	// transition. Preserve the check armed by the last non-empty state.
 ...
 * Other input is checked after the editor processes it, so only a real text
 * change dismisses the suggestion or invalidates in-flight work (F-04/F-08).
 * A genuine delete-to-empty transition schedules a re-arm (F-09). The same
 * policy is also used by GhostEditor as an editor-local fallback because OMP
 * versions can dispatch custom-editor input without invoking the extension's
 * global listener.
 */
function makeInputHandler(
	...
	return (data: string) => {
		...
		// Modal UI dialogs own their navigation and confirmation keys. Other
		// input remains user interaction and must invalidate the pending request
		// immediately, before an asynchronous dialog result can resolve.
		if (isInputSuppressed()) {
			if (isConsentDialogKey(data)) {
				markGlobalInput();
				return undefined;
			}
			dismissSuggestion(state);
		...
		// An empty editor cannot reveal whether the raw bytes edit text until the
		// focused editor processes them. Defer that decision so focus, mouse,
		// modifiers, navigation, and trailing delete input preserve the suggestion
		// and any pending re-arm; actual typing or paste dismisses afterward.
		if (editorTextBefore.length === 0) {
			markGlobalInput();
			scheduleEditorChangeCheck(state);
			return undefined;
		}

		// Text exists. Invalidate immediately, then poll until deletion settles
		// empty so the cached suggestion can re-arm.
		dismissSuggestion(state);
		state.inputGeneration += 1;
		markGlobalInput();
		state.abortInflight();
